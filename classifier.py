...
def compute_complexity_score(
    ...
    B = branchpoints
    C = curvature_turns
    L = max(skeleton_length, 1)   
    
     
    L_normalized = min(L / 100, 10)   
...
def extract_features(image: np.ndarray) -> Features:
    ...
    is_line = detect_line(shape_desc, branchpoints)    
...
def classify_rule_based(features: Features) -> Tuple[ClassResult, float, str]:
    ...
    if features.cc_count == 1 and features.skeleton_length < 400:
        return ClassResult.PUNCTUATION, 0.80, f"single_str e: skel_len={features.skeleton_length}"
